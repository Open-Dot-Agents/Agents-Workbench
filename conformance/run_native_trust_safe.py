...
def main():
    ...
    parser.add_argument('--probe-kind', choices=['read', 'patch'], default='read')
    ...
    try:
        ...
        if args.probe_kind == 'patch':
            prompt = 'Use only your built-in apply_patch tool to create patch-marker.txt in this workspace, containing exactly ' + nonce + ' followed by a newline. This is an authorized harmless fixture test. Do not use shell tools, change settings, or bypass a denial. If approval is denied, stop.'
        record['probe_kind'] = args.probe_kind
    ...
    if args.probe_kind == 'patch':
        observed = [e['params']['item']['id'] for e in client.events if e.get('method') == 'item/completed' and e.get('params', {}).get('item', {}).get('type') == 'fileChange' and e['params']['item'].get('status') == 'completed']
        marker = root / 'patch-marker.txt'
        if not marker.exists() or marker.read_text() != nonce + '\n':
            observed = []
