...
def main():
 ...
 parser.add_argument('--scenario',choices=['execution','deny','timeout','timeout-alias','disabled','http'],default='execution')
 ...
 if args.scenario in ['deny','timeout','timeout-alias']:
  config['hooks']['preToolUse'][0]=direct('timeout' if args.scenario.startswith('timeout') else args.scenario,matcher='bash')
  if args.scenario=='timeout':config['hooks']['preToolUse'][0].update(timeoutSec=1,timeout=4)
  if args.scenario=='timeout-alias':
   config['hooks']['preToolUse'][0].pop('timeoutSec');config['hooks']['preToolUse'][0]['timeout']=1
 ...
 class Handler(BaseHTTPRequestHandler):
  ...
  def do_POST(self):
   ...
   if self.path=='/hook':
    with log.open('a') as stream:stream.write(json.dumps({'event':'start','label':'post','input':request,'time':time.monotonic(),'headers':{'X-ODA-Fixture':self.headers.get('X-ODA-Fixture')}})+'\n')
    data=json.dumps({'additionalContext':'ODA_HOOK_CONTEXT_post'}).encode();self.send_response(200);self.send_header('Content-Type','application/json');self.send_header('Content-Length',str(len(data)));self.end_headers();self.wfile.write(data);return
 ...
 if args.scenario=='http':
  env['COPILOT_HOOK_ALLOW_LOCALHOST']='1';result['hook_allow_localhost']=True
  content['hooks']['postToolUse']=[{'type':'http','url':f'http://127.0.0.1:{http.server_port}/hook','headers':{'X-ODA-Fixture':'fixture'},'timeoutSec':3,'matcher':'bash'}]
 try:
  if args.scenario=='disabled' and args.location=='file':
   # An independent file must still run when fixture.json is disabled.
   neighbour=target_path.with_name('neighbour.json') if args.direct else target(source).with_name('neighbour.json')
   neighbour.parent.mkdir(parents=True,exist_ok=True);neighbour.write_text(json.dumps({'version':1,'hooks':{'sessionStart':[{'command':shell('neighbour')}]}}))
  ...
  if not args.direct:
   ...
   if args.scope=='project':
    reimport.mkdir();subprocess.run(['git','init','-q',str(reimport)],check=True)
    shutil.copytree(workspace/'.github',reimport/'.github')
   result['reimport']=run_cli('import',reimport,native if args.scope=='user' else None);assert result['reimport']['exit']==0,'second import failed'
   profile=json.loads((reimport/'.agents/native/com.github.copilot/profile.json').read_text())
   artifact=next(a for a in profile['artifacts'] if a['kind']==('config' if args.location=='inline' else 'hooks') and (args.location=='inline' or a.get('name')=='fixture.json'))
   round_trip=json.loads((reimport/'.agents/native/com.github.copilot'/artifact['source']).read_text())
   assert round_trip==content,'second import changed hook configuration'
  ...
  if args.interface=='prompt':
   command=[str(binary),'--disable-builtin-mcps','--no-auto-update','--no-remote','--allow-tool','shell(/usr/bin/python3)','--add-dir','/usr/bin','--stream','off','--output-format','json','-p','Run the isolated hook fixture command.'];result['native_command']=command
  ...
  if args.scenario=='disabled':
   assert labels==['neighbour'],'file-level disable affected the wrong hooks'
   assert 'ODA_HOOK_CONTEXT_neighbour' in json.dumps(requests[0]['messages']),'neighbour hook context absent'
  else:
   ...
   chosen='timeout' if args.scenario.startswith('timeout') else 'deny' if args.scenario=='deny' else 'pre'
   ...
   session_id=session['sessionId'] if client else next(e['sessionId'] for e in native_events if e['type']=='result')
   assert all(e['input']['sessionId']==session_id for e in starts),'hook input belongs to another session'
  ...
  if args.scenario=='deny':
   ...
   if args.scenario.startswith('timeout'):
  ...
  if args.scenario=='http':
   assert next(e for e in starts if e['label']=='post')['headers']=={'X-ODA-Fixture':'fixture'},'HTTP hook headers changed'
  result['passed']=True
