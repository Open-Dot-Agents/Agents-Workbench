...
child = None
if scenario in ['background-timeout', 'background-detached-timeout', 'background-unsubscribe', 'background-shutdown', 'background-archive']:
    child = subprocess.Popen(['/usr/bin/python3', '-c', 'import time; from pathlib import Path; time.sleep(3); Path(' + repr(str(pathlib.Path(log).with_suffix('.child-effect'))) + ').write_text("ODA_CHILD_EFFECT")'], start_new_session=scenario == 'background-detached-timeout')
def record(event):
    with pathlib.Path(log).open('a') as stream:
        stream.write(json.dumps({'event': event, 'input': payload, 'pid': os.getpid(),
                                ...
                                'child_pgid': os.getpgid(child.pid) if child else None}) + '\n')
record('start')
if scenario.startswith('background'):
    ...
    if scenario in ['background-spill', 'background-unlimited']:
...
def main():
    ...
    parser.add_argument('--scenario', choices=['background-next', 'background-active',
                        'background-deny', 'background-timeout', 'background-detached-timeout', 'background-unsubscribe', 'background-shutdown', 'background-archive', 'background-spill', 'background-unlimited',
    ...
    root = Path(tempfile.mkdtemp(prefix='oda-codex-background-'))
    ...
    if args.scenario == 'background-spill':
        handler['additionalContextLimit'] = 128
    if args.scenario == 'spill-default':
        handler.pop('additionalContextLimit')
    if args.scenario in ['background-timeout', 'background-detached-timeout']:
        handler['timeout'] = 1
    handlers = [handler]
    ...
    def check_context(received):
        ...
        if args.scenario in ['unlimited', 'background-unlimited']:
    ...
    def check_execution():
        ...
        if background:
            ...
            if initial[0]['child_pid']:
                detached = args.scenario == 'background-detached-timeout'
                assert (initial[0]['child_pgid'] != initial[0]['pgid']) == detached, 'fixture process-group setup mismatch'
            ...
            if args.scenario in ['background-next', 'background-deny', 'background-spill', 'background-unlimited']:
            ...
            if args.scenario in ['background-unsubscribe', 'background-shutdown', 'background-archive']:
                ...
                if args.scenario in ['background-timeout', 'background-detached-timeout']:
                    ...
                    assert process_state(initial[0]['pid']) in [None, 'Z'], 'native timeout left hook alive'
                    assert len(hook_log()) == 1, 'timed-out hook completed'
                    assert 0.8 <= result['observed_timeout_seconds'] < 3, 'one-second timeout not applied'
                    if args.scenario == 'background-detached-timeout':
                        assert process_state(initial[0]['child_pid']) not in [None, 'Z'], 'detached fixture child did not survive'
                        effect_deadline = time.monotonic() + 4
                        while not log.with_suffix('.child-effect').exists() and time.monotonic() < effect_deadline:
                            time.sleep(0.02)
                        assert log.with_suffix('.child-effect').read_text() == 'ODA_CHILD_EFFECT', 'detached fixture effect absent'
                        result['detached_child_effect'] = log.with_suffix('.child-effect').read_text()
                    else:
                        assert process_state(initial[0]['child_pid']) in [None, 'Z'], 'native timeout left attached child alive'
                        assert not log.with_suffix('.child-effect').exists(), 'timed-out attached child completed'
                else:
                    ...
                    if args.scenario in ['background-spill', 'background-unlimited']:
                        check_context(model_text(requests[-1]['input']))
                    else:
            ...
            check_context(model_text(requests[0]['input']))
        if background and args.scenario != 'background-shutdown':
            result['final_loaded_threads'] = client.response(client.request('thread/loaded/list', {}), time.monotonic() + 10)
        ...
        if background:
            ...
            for entry in hook_log():
                ...
                assert any(e.get('method') == 'item/completed' and
                           e.get('params', {}).get('item', {}).get('type') == 'commandExecution' and
                           e['params']['item'].get('id') == entry['input']['tool_use_id'] and
                           e['params']['item'].get('status') == 'completed' for e in client.events), 'hook input has no matching native command completion'
        else:
    ...
    try:
        ...
        native_handlers = [h for e in result['execution_hooks_list']['data'] for h in e['hooks']]
        assert len(native_handlers) == len(handlers), 'reloaded hook count mismatch'
        for observed, configured in zip(native_handlers, handlers):
            assert observed['async'] == configured['async'], 'native async value changed'
            assert observed['additionalContextLimit'] == configured.get('additionalContextLimit'), 'native context limit changed'
            assert observed['timeoutSec'] == configured['timeout'], 'native timeout changed'
