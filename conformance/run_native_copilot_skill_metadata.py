...
def terminal(binary, workspace, env, prompt, result, decision):
    """Observe the picker, then submit one isolated interactive turn."""
    import pexpect
    child = pexpect.spawn(str(binary), ['--disable-builtin-mcps', '--no-auto-update', '--no-remote'],
                          cwd=str(workspace), env={**env, 'TERM': 'xterm-256color'},
                          encoding='utf-8', dimensions=(40, 140), timeout=1)
    phase = result['terminal'] = {'raw_output': '', 'phases': {}}
    pending = ''

    def read(seconds):
        nonlocal pending
        start, offset = time.monotonic(), len(phase['raw_output'])
        while time.monotonic()-start < seconds and child.isalive():
            try:
            ...
            pending = pending[-16:]
        return plain_terminal(phase['raw_output'][offset:])

    try:
        phase['phases']['startup'] = read(5)
        child.send('/fixture-hint')
        phase['phases']['hint'] = read(2)
        child.send('\t')
        phase['phases']['hint_selected'] = read(1)
        child.send('\x1b')
        child.sendcontrol('u')
        read(.5)
        child.send(prompt)
        child.send('\r')
        phase['phases']['turn'] = read(12)
        phase['effect_before_decision'] = (workspace / 'skill-effect.txt').exists()
        if decision == 'allow':
            assert 'Do you want to run this command?' in phase['phases']['turn']
            assert result['shell_command'] in phase['phases']['turn']
            assert 'Allow directory access' not in phase['phases']['turn']
            child.send('\r')  # One approval for the exact fixture command.
        else:
            child.send('\x1b')
        phase['decision'] = decision
        phase['phases']['decision_result'] = read(4)
        ...
        try:
            ...
            if child.isalive():
                raise
            child.close(force=True)
        phase['alive_after_teardown'] = child.isalive()
...
def main():
    ...
    parser.add_argument('--interface', choices=['acp', 'cli', 'tui'], default='acp')
    parser.add_argument('--trigger', choices=['model', 'user'], default='model')
    parser.add_argument('--command-style', choices=['python', 'builtin'], default='python')
    parser.add_argument('--decision', choices=['deny', 'allow'], default='deny')
    parser.add_argument('--output', type=Path, required=True)
    args = parser.parse_args()
    if args.decision == 'allow' and (args.interface != 'tui' or args.command_style != 'builtin'):
        parser.error('allow control requires the TUI and the isolated shell built-in')
    ...
    result['command_style'] = args.command_style
    ...
    shell_command = ('/usr/bin/python3 '+str(probe) if args.command_style == 'python'
                     else 'printf AGENTS_SKILL_EFFECT > '+shlex.quote(str(marker)))
    ...
    class Model(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if args.invoke and args.trigger == 'model' and len(requests) == 1:
                ...
                call = ('bash', {'command': shell_command, 'description': 'Write isolated skill effect'})
            if call:
    ...
    def command(argv, cwd=workspace, expect_failure=False):
        ...
        assert (run.returncode != 0) if expect_failure else (run.returncode == 0), json.dumps(row)
        return row
    ...
    try:
        ...
        if args.scope == 'user':
            ...
            shutil.copytree(root / 'skills', source_home / 'skills')
            canonical = base / 'imported'
            command([cli, 'import', '--vendor', 'copilot', '--experimental', '--scope', 'user', '--root', canonical, '--native-home', source_home])
            root = canonical / '.agents'
            result['imported_hashes'] = {name: sha(root / 'skills' / ('fixture-'+name) / 'SKILL.md') for name in sources}
            assert result['imported_hashes'] == {name: s['sha256'] for name, s in sources.items()}
        ...
        def snapshot():
            return {str(p.relative_to(base)): sha(p) for directory in (canonical, home, base / 'state')
                    if directory.exists() for p in directory.rglob('*') if p.is_file()}
        before_apply = snapshot()
        applied = command([cli, 'apply', *flags], expect_failure=args.cases == 'malformed')
        result['projection_refused'] = applied['exit_code'] != 0
        result['refusal_unchanged'] = snapshot() == before_apply if result['projection_refused'] else None
        result['state_unchanged_by_apply'] = sha(home / 'config.json') == state_before
        assert result['state_unchanged_by_apply']
        target = root / 'skills' if args.scope == 'project' else home / 'skills'
        if args.cases == 'malformed':
            assert result['refusal_unchanged'] and not result['plan']['applicable'] and result['plan']['actions'] == []
            # Direct native discovery is a separate probe after adapter refusal.
            # The setup copy is not adapter projection or activation evidence.
            if args.scope == 'user': shutil.copytree(root / 'skills', target)
            result['native_setup_after_refusal'] = True
        elif args.scope == 'user':
            again = base / 'reimported'
            command([cli, 'import', '--vendor', 'copilot', '--experimental', '--scope', 'user', '--root', again, '--native-home', home])
            result['reimported_hashes'] = {name: sha(again / '.agents/skills' / ('fixture-'+name) / 'SKILL.md') for name in sources}
        result['target_hashes'] = {name: sha(target / ('fixture-'+name) / 'SKILL.md') for name in sources}
        assert all(result['target_hashes'][name] == item['sha256'] for name, item in sources.items())
        result['source_unchanged_by_apply'] = all(sha(root / 'skills' / ('fixture-'+name) / 'SKILL.md') == item['sha256'] for name, item in sources.items())
        assert result['source_unchanged_by_apply']
        ...
        if args.interface == 'acp':
            ...
            terminal(binary, workspace, env, prompt_text, result, args.decision)
        assert requests and nonce in json.dumps(requests)
        result['catalog_descriptions'] = {name: 'AGENTS_SKILL_DESCRIPTION_'+name in json.dumps(requests[0]) for name in cases}
        ...
        if result.get('probe_completed'):
            try:
                result['observations'] = evaluate(result)
                result['passed'] = True
            except Exception:
                result['error'] = traceback.format_exc()
        output.parent.mkdir(parents=True, exist_ok=True)
        output.with_suffix('.runner.py').write_bytes(Path(__file__).read_bytes())
        output.with_suffix('.verifier.py').write_bytes(Path(__file__).with_name('verify_copilot_skill_metadata.py').read_bytes())
