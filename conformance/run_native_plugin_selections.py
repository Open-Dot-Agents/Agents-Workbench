...
def main():
    ...
    def discover():
        if args.vendor == 'copilot':
            ...
            skills = invoke([str(binary), 'skill', 'list', '--json'])
            return {'resources': json.loads(completed['stdout']), 'skills': json.loads(skills['stdout'])}
    ...
    try:
        ...
        enabled = result['enabled_discovery']
        if args.vendor == 'codex':
            records = [s for item in enabled['data'] if item['cwd'] == str(workspace)
                       for s in item['skills'] if s.get('pluginId') == 'fixture@oda-fixture']
            assert len(records) == 1 and records[0]['scope'] == 'user' and records[0]['enabled'] is True, 'Codex plugin skill identity or scope differs'
            assert records[0]['name'] == 'fixture:fixture'
            assert all(not item['errors'] for item in enabled['data'])
            installed_skill = Path(records[0]['path'])
            installed_package = home / 'plugins/cache/oda-fixture/fixture/1.0.0'
        else:
            records = [p for p in enabled['resources']['plugins'] if p['kind'] == 'plugin' and p['name'] == 'fixture']
            assert len(records) == 1 and records[0]['source'] == 'live-marketplace:oda-fixture'
            assert records[0]['scope'] == 'user' and records[0]['version'] == '1.0.0'
            assert records[0]['enabled'] is True and not enabled['resources']['errors']
            assert Path(records[0]['installedFrom']) == market
            installed_package = plugin
            skills = [s for s in enabled['skills'] if s['name'] == 'fixture']
            assert len(skills) == 1
            assert skills[0]['source'] == 'plugin' and skills[0]['enabled'] is True
            installed_skill = Path(skills[0]['path']) / 'SKILL.md'
            assert not (home / 'installed-plugins').exists(), 'local marketplace unexpectedly copied packages'
        assert installed_skill == installed_package / 'skills/fixture/SKILL.md'
        assert {name: sha(installed_package / name) for name in package_hashes} == package_hashes
        result['installed_package'] = str(installed_package)
        result['installed_package_hashes'] = {name: sha(installed_package / name) for name in package_hashes}
        ...
        disabled = result['disabled_discovery']
        if args.vendor == 'codex':
            assert not any(s.get('pluginId') == 'fixture@oda-fixture' for item in disabled['data'] for s in item['skills'])
            assert all(not item['errors'] for item in disabled['data'])
        else:
            records = [p for p in disabled['resources']['plugins'] if p['kind'] == 'plugin' and p['name'] == 'fixture']
            assert len(records) == 1 and records[0]['enabled'] is False
            assert records[0]['source'] == 'live-marketplace:oda-fixture' and Path(records[0]['installedFrom']) == market
            assert not disabled['resources']['errors']
            assert not any(s['name'] == 'fixture' for s in disabled['skills'])
        assert {name: sha(installed_package / name) for name in package_hashes} == package_hashes, 'disable removed or changed the installed package'
