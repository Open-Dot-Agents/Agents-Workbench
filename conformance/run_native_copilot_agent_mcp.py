...
def main():
 ...
 imported=None
 if not args.direct:
  source=root/'source';source.mkdir();subprocess.run(['git','init','-q',str(source)],check=True)
  seed=(source/'.github' if args.scope=='project' else source)/'agents'/args.filename
  seed.parent.mkdir(parents=True);seed.write_text(configuration)
  import_command=[str(cli),'import','--vendor','copilot','--root',str(source),'--experimental','--scope',args.scope]
  if args.scope=='user':import_command+=['--native-home',str(source)]
  imported=subprocess.run(import_command,env=env,capture_output=True,text=True)
  if imported.returncode==0:shutil.copytree(source/'.agents',canonical/'.agents')
 ...
 try:
  if imported:assert imported.returncode==0,'import failed'
  ...
  if not args.direct:
   ...
   if args.scope=='project':
    ...
    shutil.copytree(canonical/'.github',reimport/'.github')
   import_command=[str(cli),'import','--vendor','copilot','--root',str(reimport),'--experimental','--scope',args.scope]
   if args.scope=='user':import_command+=['--native-home',str(native)]
   again=subprocess.run(import_command,env=env,capture_output=True,text=True)
   result['reimport']={'exit':again.returncode,'stdout':again.stdout,'stderr':again.stderr}
   assert again.returncode==0,'second import failed'
   assert (reimport/'.agents/native/com.github.copilot/agent'/args.filename).read_text()==configuration,'second import changed agent bytes'
  ...
  client=Client(command,workspace,env,'deny','');deadline=time.monotonic()+50
