...
def main():
    ...
    result = {'fixture': str(root), 'native_version': '1.0.83', 'native_sha256': sha(binary),
              ...
              'commands': [], 'phases': [], 'environment': env, 'full_adapter_support': False,
    ...
    def terminal(phase):
        ...
        try:
            ...
            # Preserve observations even when native teardown fails. Pexpect's
            # short post-SIGKILL wait can expire before waitpid reports exit.
            phase.update(raw_output=raw, output=plain_terminal(raw))
            try:
                child.close(force=True)
            except pexpect.ExceptionPexpect as error:
                phase['teardown_first_error'] = str(error)
                ...
                phase.update(elapsed_seconds=time.monotonic() - start,
                             exit_status=child.exitstatus, signal_status=child.signalstatus,
                             alive_after_teardown=child.isalive(),
                             teardown='pexpect close(force=True); on failure poll the same child for up to five seconds')
    ...
    try:
        ...
        result['status_command_source'] = script.read_text()
