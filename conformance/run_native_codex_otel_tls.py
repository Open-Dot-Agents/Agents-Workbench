...
def certificate(directory, name, issuer=None, server=False, key_type='ec'):
    ...
    builder = (x509.CertificateBuilder().subject_name(subject)
               ...
               .add_extension(x509.AuthorityKeyIdentifier.from_issuer_public_key(issuer[1].public_key() if issuer else key.public_key()), critical=False)
               .add_extension(x509.KeyUsage(digital_signature=True, content_commitment=False,
                   key_encipherment=issuer is not None and key_type == 'rsa', data_encipherment=False,
                   key_agreement=False, key_cert_sign=issuer is None, crl_sign=issuer is None,
...
def main():
    ...
    parser.add_argument('--signal', choices=['both', 'exporter', 'trace_exporter', 'metrics_exporter'], default='both')
    ...
    if args.signal != 'both' and not args.expect_identity_failure:
        parser.error('individual signals are identity-failure controls')
    ...
    result = {'fixture': str(root), 'native_version': '0.154.0', 'native_sha256': sha(binary), 'paths': args.paths,
              ...
              'signal': args.signal,
              'tls_control_versions': {'openssl': ssl.OPENSSL_VERSION, 'cryptography': cryptography_version},
              ...
              'limitations': ['Local OTLP HTTP JSON and native HTTP client-identity failure probes only.',
    ...
    def run_native(home, label):
        ...
        if args.expect_identity_failure:
            error = 'Could not create otel exporter: ' + ('invalid OTLP metrics configuration: ' if args.signal == 'metrics_exporter' else '') + 'builder error'
            assert not phase['exports'] and error in phase['stderr'], 'native identity limitation changed'
    ...
    try:
        ...
        selected_signals = [('exporter', 'logs'), ('trace_exporter', 'traces')] if args.signal == 'both' else [(args.signal, {'exporter': 'logs', 'trace_exporter': 'traces', 'metrics_exporter': 'metrics'}[args.signal])]
        text = '[otel]\nlog_user_prompt = false\n'
        for kind in ('exporter', 'trace_exporter', 'metrics_exporter'):
            if kind not in dict(selected_signals): text += kind + ' = "none"\n'
        for kind, suffix in selected_signals:
