...
def main():
    ...
    parser.add_argument('--skill-reference', action='store_true')
    ...
    base = Path(tempfile.mkdtemp(prefix='agents-role-reference-'))
    source, target, workspace, canonical = [base / name for name in ('source', 'target', 'workspace', 'canonical')]
    if args.skill_reference: target = base / 'relocated/target'
    for path in (source, target, workspace, canonical): path.mkdir(mode=0o700, parents=True)
    ...
    result = {'passed': False, 'reference': args.reference, 'scope': args.scope, 'skill_reference': args.skill_reference, 'fixture': str(base), 'native_version': '0.154.0',
...
''')
    ...
    if args.skill_reference:
        skill = (workspace / '.agents' if args.scope == 'user' else base / 'project-home') / 'skills/fixture/SKILL.md'
        skill.parent.mkdir(parents=True)
        skill.write_text('---\nname: fixture\ndescription: AGENTS_REFERENCED_SKILL_DESCRIPTION\n---\nUse this fixture.\n')
        role.write_text(role.read_text()+'\nskills.config=[{path='+json.dumps(os.path.relpath(skill, role.parent))+',enabled=false}]\n')
...
'''
    ...
    def native(label, home):
        ...
        try:
            ...
            if args.skill_reference:
                ...
                assert all('AGENTS_REFERENCED_SKILL_DESCRIPTION' not in json.dumps(r['body'].get('input', [])) for r in children), 'role skill reference changed location after '+label
    ...
    try:
        ...
        if args.reference == 'managed':
            if args.skill_reference:
                projected_role = tomllib.loads((target_config_dir / 'agents/fixture.toml').read_text())
                assert projected_role['skills']['config'][0]['path'] == str(skill)
                result['projected_role'] = projected_role
            else:
                assert sha(target_config_dir / 'agents/fixture.toml') == original_role_hash
