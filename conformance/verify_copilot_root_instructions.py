...
def check_phase(phase, case):
    ...
    assert 'AGENTS_ROOT_INSTRUCTION_MARKER' in context
...
def verify():
    ...
    for case in ('root', 'identical', 'distinct', 'reference'):
        path = BASE/f'copilot-root-instructions-{case}-user-instructions.json'
        ...
        assert result['implementation_sha256'] == implementation
        assert result['runner_sha256'] == sha(path.with_suffix('.runner.py')) == sha(ROOT/'WORKBENCH/conformance/run_native_copilot_root_instructions.py')
        assert result['helper_sha256'] == sha(ROOT/'WORKBENCH/conformance/run_native_approvals.py')
    ...
    print('PASS: four root instruction cases; eight native sessions; no support promotion')
