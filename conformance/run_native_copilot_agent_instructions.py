#!/usr/bin/env python3
"""Check fixed native instruction paths and relative references through relocation."""
import argparse
...
def main():
    ...
    parser.add_argument('--case', choices=['combined', 'fallback', 'dot-only'], required=True)
    parser.add_argument('--cwd-subdir', choices=['.', '.claude'], default='.')
    ...
    definitions = {
        'AGENTS.md': 'AGENTS_ROOT_INSTRUCTION_MARKER\n@root-policy.md\n',
        'CLAUDE.md': 'AGENTS_CLAUDE_INSTRUCTION_MARKER\n@claude-policy.md\n',
        '.claude/CLAUDE.md': 'AGENTS_DOT_CLAUDE_INSTRUCTION_MARKER\n@policy.md\n',
        'GEMINI.md': 'AGENTS_GEMINI_INSTRUCTION_MARKER\n@gemini-policy.md\n',
        '.github/copilot-instructions.md': 'AGENTS_COPILOT_INSTRUCTION_MARKER\n@policy.md\n',
    }
    if args.case == 'fallback': del definitions['CLAUDE.md']
    if args.case == 'dot-only': definitions = {name: body for name, body in definitions.items() if name == '.claude/CLAUDE.md'}
    references = {
        'root-policy.md': 'AGENTS_ROOT_REFERENCED_POLICY\n@child-policy.md\n',
        'child-policy.md': 'AGENTS_CHILD_REFERENCED_POLICY\n',
        'claude-policy.md': 'AGENTS_CLAUDE_REFERENCED_POLICY\n',
        '.claude/policy.md': 'AGENTS_DOT_CLAUDE_REFERENCED_POLICY\n',
        '.github/policy.md': 'AGENTS_COPILOT_REFERENCED_POLICY\n',
        'gemini-policy.md': 'AGENTS_GEMINI_REFERENCE_MUST_NOT_LOAD\n',
    }
    ...
    def native(label, workspace):
        ...
        workspace = workspace/args.cwd_subdir
        ...
        phase['instruction_markers'] = {name: body.splitlines()[0] in system for name, body in definitions.items()}
        phase['reference_markers'] = {name: body.splitlines()[0] in system for name, body in references.items()}
        assert all(value == (name != '.claude/CLAUDE.md' or args.cwd_subdir == '.claude') for name, value in phase['instruction_markers'].items()), phase['instruction_markers']
        expected_references = set()
        if 'AGENTS.md' in definitions: expected_references.update({'root-policy.md', 'child-policy.md'})
        if 'CLAUDE.md' in definitions: expected_references.add('claude-policy.md')
        if '.github/copilot-instructions.md' in definitions: expected_references.add('.github/policy.md')
        if args.cwd_subdir == '.claude': expected_references.add('.claude/policy.md')
        assert all(value == (name in expected_references) for name, value in phase['reference_markers'].items()), phase['reference_markers']
    ...
    try:
        ...
        assert (source/'.agents/AGENTS.md').read_text() == definitions.get('.github/copilot-instructions.md', 'Use the selected native profile.\n')
        for name, body in definitions.items():
            if name != '.github/copilot-instructions.md':
                assert (source/'.agents/native/com.github.copilot/agent-instructions'/name).read_text() == body
        shutil.copytree(source/'.agents', target/'.agents')
        result['plan'] = json.loads(adapter('plan', target)['stdout'])
        adapter('apply', target)
        for name, body in definitions.items(): assert (target/name).read_text() == body
        native('relocated', target)
        adapter('import', target)
        assert (target/'.agents/AGENTS.md').read_text() == definitions.get('.github/copilot-instructions.md', 'Use the selected native profile.\n')
        for name, body in definitions.items():
            if name != '.github/copilot-instructions.md':
                assert (target/'.agents/native/com.github.copilot/agent-instructions'/name).read_text() == body
        result['references_unchanged'] = all((workspace/name).read_text() == body for workspace in (source, target) for name, body in references.items())
