...
import argparse,json,shlex,shutil,subprocess,tempfile,threading,time,tomllib
...
def record(event):
 with pathlib.Path(sys.argv[1]).open('a') as out:out.write(json.dumps({'event':event,'label':sys.argv[2],'input':payload,'cwd':os.getcwd(),'time':time.monotonic(),'pid':os.getpid()})+'\n')
record('start')
...
def main():
 ...
 p.add_argument('--trust',choices=['unreviewed','bypass','preconfigured'],default='preconfigured');p.add_argument('--scenario',choices=['execution','deny','timeout','disabled-field','modified'],default='execution')
 ...
 try:
  ...
  if args.scenario=='modified':
   assert not args.direct and args.location=='file' and args.trust=='preconfigured','modified scenario requires a projected trusted file'
   result['configuration_before_change']=json.loads(json.dumps(config))
   config['hooks']['SessionStart'][0]['hooks'][0]['command']+=' ODA_CHANGED'
   (workspace/'.agents/native/com.openai.codex/hooks.json').write_text(json.dumps(config))
   result['apply_modified']=run_cli('apply',workspace,home if args.scope=='user' else None);assert result['apply_modified']['exit']==0,'modified hook apply failed'
   assert sha(home/'config.toml')==trusted_hash,'modified apply changed native trust'
   client.close();result['pre_modification_events']=client.events
   client=Client(command,workspace,env,'deny',str(probe));deadline=time.monotonic()+40
   client.response(client.request('initialize',{'clientInfo':{'name':'oda-hooks','version':'0.1.0'},'capabilities':{'experimentalApi':True}}),deadline);client.send({'method':'initialized','params':{}})
   result['modified_hooks_list']=client.response(client.request('hooks/list',{'cwds':[str(workspace)]}),deadline)
   statuses=[h['trustStatus'] for entry in result['modified_hooks_list']['data'] for h in entry['hooks']]
   assert statuses.count('modified')==1 and statuses.count('trusted')==3,'changed definition did not require new review'
  if not args.direct:
   reimport=root/'reimport'
   if args.scope=='project':
    reimport.mkdir();subprocess.run(['git','init','-q',str(reimport)],check=True);shutil.copytree(workspace/'.codex',reimport/'.codex')
   result['reimport']=run_cli('import',reimport,home if args.scope=='user' else None);assert result['reimport']['exit']==0,'second import failed'
   namespace=reimport/'.agents/native/com.openai.codex';profile=json.loads((namespace/'profile.json').read_text());artifact=next(a for a in profile['artifacts'] if a['kind']==('config' if args.location=='inline' else 'hooks'))
   imported=(tomllib.loads if args.location=='inline' else json.loads)((namespace/artifact['source']).read_text())
   assert imported['hooks']==config['hooks'],'hook values changed during second import'
   assert 'state' not in imported['hooks'],'native trust state imported'
  ...
  if args.trust=='unreviewed':assert not hooks,'unreviewed hooks executed'
   ...
   labels=[e['label'] for e in starts];assert ('start' in labels)==(args.scenario!='modified') and 'excluded' not in labels,'hook discovery or matcher mismatch'
   assert all(e['cwd']==str(workspace) and e['input']['session_id']==thread['thread']['id'] for e in starts),'hook cwd or session mismatch'
   assert ('ODA_CODEX_HOOK_start' in json.dumps(requests[0]['input']))==(args.scenario!='modified'),'session context mismatch'
  ...
  if args.scenario=='timeout' and args.trust!='unreviewed':
   pre=next(e for e in starts if e['label']=='timeout');post=next(e for e in starts if e['label']=='post')
   assert 0.8<post['time']-pre['time']<4,'native hook timeout was not applied'
   assert not any(e['label']=='timeout' and e['event']=='end' for e in hooks),'timed-out hook finished'
   process=Path('/proc')/str(pre['pid'])/'stat'
   result['timed_out_process_state']=process.read_text().split(') ',1)[1].split()[0] if process.exists() else 'absent'
   assert result['timed_out_process_state'] in ['absent','Z'],'timed-out hook process remains active'
  result['passed']=True
