...
def main():
    ...
    parser.add_argument('--analytics', choices=['on', 'off'], default='on')
    parser.add_argument('--credentials', choices=['none', 'inline'], default='none')
    ...
    result = {'native_version': '0.154.0', 'native_sha256': sha(binary), 'runner_sha256': sha(__file__),
              'transport': args.transport, 'tls': args.tls, 'adapter': args.adapter, 'signal': args.signal, 'analytics': args.analytics, 'credentials': args.credentials,
    ...
    def collect(signal, message, metadata, serial):
        ...
        authorization = entry['headers'].pop('authorization', entry['headers'].pop('Authorization', None))
        entry['authorized'] = authorization == 'Bearer ODA_OTLP_SYNTHETIC'
    ...
    class Model(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            wire.append({'path': self.path, 'body': json.loads(self.rfile.read(int(self.headers['Content-Length'])))})
    ...
    def invoke(command, native_home=source, cwd=workspace, check=True):
        ...
        if check: assert run.returncode == 0, json.dumps(entry)
        return entry
    ...
    def run_native(home, label):
        ...
        assert run['exit_code'] == 0 and any(e['type'] == 'turn.completed' for e in events), 'no completed native turn'
        thread = next(e['thread_id'] for e in events if e['type'] == 'thread.started')
        response_requests = [r for r in phase['requests'] if r['path'].endswith('/responses')]
        phase['model_response_requests'] = len(response_requests)
        assert response_requests and all(nonce in json.dumps(r['body']) for r in response_requests), 'model prompt missing'
        ...
        if args.analytics == 'off': expected.discard('metrics')
        if args.credentials == 'inline' and label == 'relocated': expected = set()
        ...
        if args.credentials == 'inline':
            assert all(e['authorized'] for e in phase['exports']), 'exporter lost authentication'
        ...
        resources = [r['resource'] for e in phase['exports'] for name in ('resourceLogs', 'resourceSpans', 'resourceMetrics') for r in e['body'].get(name, [])]
        assert all(attributes(r.get('attributes', [])).get('env') == 'oda-transport-probe' for r in resources), 'resource environment missing'
        ...
        if 'metrics' in expected:
            ...
            counters = [p for m in metrics if m['name'] == 'codex.api_request' for p in m['sum']['dataPoints']]
            matching = [p for p in counters if attributes(p.get('attributes', [])).get('model') == 'fixture-model'
                        and attributes(p.get('attributes', [])).get('status') == '200'
                        and attributes(p.get('attributes', [])).get('success') == 'true']
            phase['api_request_count'] = sum(int(p.get('asInt', 0)) for p in matching)
            assert phase['api_request_count'] == len(response_requests), 'API metric does not match model requests'
    ...
    try:
        ...
        text = '[analytics]\nenabled = '+str(args.analytics == 'on').lower()+'\n\n[otel]\nenvironment = "oda-transport-probe"\nlog_user_prompt = false\n'
        ...
        for signal, (kind, _, _) in SIGNALS.items():
            ...
            if args.credentials == 'inline': headers += ', "Authorization"="Bearer ODA_OTLP_SYNTHETIC"'
        ...
        if args.adapter:
            ...
            if args.credentials == 'inline':
                for signal, (kind, _, _) in SIGNALS.items():
                    if args.signal == 'all' or args.signal == signal:
                        assert imported['otel'][kind] == 'none', 'credential removal permits an active or default exporter'
                assert 'ODA_OTLP_SYNTHETIC' not in (owner / filename).read_text(), 'credential copied into canonical configuration'
