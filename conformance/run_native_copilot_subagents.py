    ...
    try:
        ...
        if args.case == 'disabled':
            ...
            if args.case == 'limits':
                ...
                result['context_tier_observation'] = [e['data'].get('contextTier') for e in configured]
                # Native omits the default tier in this event. Keep that
                # absence in the record; require the explicit override value.
                assert all(e['data'].get('contextTier', 'default') == tier for e in configured), 'configured context-tier event differs'
