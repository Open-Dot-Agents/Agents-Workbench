...
def main():
    ...
    class Model(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            call = None
            if args.invoke and len(requests) == 1:
                call = ('skill', {'skill': 'fixture-'+args.invoke})
            elif args.invoke and len(requests) == 2:
    ...
    try:
        ...
        if args.interface == 'acp':
            ...
            result['prompt'] = client.response(client.request('session/prompt', {'sessionId': session['sessionId'], 'prompt': [{'type': 'text', 'text': nonce}]}), deadline)
            assert result['prompt']['stopReason'] == 'end_turn'
        else:
            result['cli_turn'] = command([binary, '--disable-builtin-mcps', '--no-auto-update', '--no-remote', '-p', nonce])
        ...
        result['invoked_body_in_model'] = bool(args.invoke) and any('AGENTS_SKILL_BODY_'+args.invoke in json.dumps(r['messages']) for r in requests[1:])
