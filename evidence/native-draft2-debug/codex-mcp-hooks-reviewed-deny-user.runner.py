...
MCP = r'''
import json, pathlib, sys, time, threading
lock = threading.Lock()
...
def handle(request):
    ...
    if method == 'initialize':
        ...
        if request['params']['name'] != 'record':
            result = {'isError': True, 'content': [{'type': 'text', 'text': 'ODA_MCP_TOOL_MISSING'}]}
...
def main():
    ...
    try:
        ...
        assert all(h['handlerType'] == 'mcpTool' and h['server'] in ['oda_hook', 'missing']
                   and h['tool'] in ['record', 'missing'] for h in definitions), 'unexpected MCP definition'
        ...
        if args.scenario == 'timeout':
            # The fixture server deliberately ignores cancellation. Observe its late
            # result before shutdown; an MCP timeout does not prove tool termination.
            late_deadline = time.monotonic() + 6
            while time.monotonic() < late_deadline:
                observed = [json.loads(line) for line in log.read_text().splitlines()]
                if len([e for e in observed if e.get('event') == 'result']) == 2:
                    break
                time.sleep(0.05)
        ...
        if args.scenario == 'unreviewed':
            ...
            expected_count = 1 if args.scenario in ['deny', 'missing-server', 'template-missing'] else 2
            assert len(calls) == expected_count, 'unexpected MCP call count'
            for call in calls:
                values = call['params']['arguments']
                expected_tool = 'missing' if args.scenario == 'missing-tool' and values['event'] == 'PreToolUse' else 'record'
                assert call['params']['name'] == expected_tool, 'unexpected MCP tool'
        ...
        hook_runs = [e['params'] for e in client.events if e.get('method') == 'hook/completed']
        result['completed_hook_runs'] = hook_runs
        assert len(hook_runs) == (0 if args.scenario == 'unreviewed' else 1 if args.scenario == 'deny' else 2), 'unexpected native hook count'
        starts = {e['params']['run']['id'] for e in client.events if e.get('method') == 'hook/started'}
        for entry in hook_runs:
            run = entry['run']
            assert entry['threadId'] == thread['thread']['id'], 'native hook thread mismatch'
            assert entry['turnId'] == result['completed_turn']['id'], 'native hook turn mismatch'
            assert run['id'] in starts and run['id'].endswith(':fixture-hook-command'), 'native hook call correlation absent'
            assert run['sourcePath'] == str(native_target) and run['handlerType'] == 'mcpTool', 'native hook source mismatch'
            assert run['executionMode'] == 'sync', 'unexpected MCP hook execution mode'
            assert run['statusMessage'] == 'ODA_MCP_LOADING_' + ('PreToolUse' if run['eventName'] == 'preToolUse' else 'PostToolUse'), 'native status metadata mismatch'
        if hook_runs:
            pre = hook_runs[0]['run']
            expected_status = 'blocked' if args.scenario == 'deny' else 'failed' if args.scenario in ['missing-server', 'missing-tool', 'error', 'timeout', 'template-missing'] else 'completed'
            assert pre['status'] == expected_status, 'native pre-hook status mismatch'
            diagnostics = {'missing-server': "unknown MCP server 'missing'", 'missing-tool': 'ODA_MCP_TOOL_MISSING', 'error': 'ODA_MCP_HOOK_ERROR', 'template-missing': 'was not found', 'timeout': 'tool call failed'}
            if args.scenario in diagnostics:
                assert diagnostics[args.scenario] in json.dumps(pre['entries']), 'native failure diagnostic absent'
            if args.scenario != 'deny':
                assert hook_runs[1]['run']['status'] == 'completed', 'native post-hook not complete'
        if args.scenario == 'timeout':
            assert 800 <= pre['durationMs'] < 3000, 'one-second MCP hook timeout not applied'
            late = next(e for e in events if e.get('event') == 'result' and e['request_id'] == calls[0]['id'])
            assert calls[1]['recorded_at'] - calls[0]['recorded_at'] < 4, 'command waited for late MCP result'
            assert late['recorded_at'] - calls[0]['recorded_at'] >= 4.5, 'late MCP effect not observed'
            assert 'ODA_MCP_HOOK_PreToolUse' not in json.dumps(requests[-1]['input']), 'late hook context reached the completed turn'
