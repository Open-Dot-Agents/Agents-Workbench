...
def main():
    ...
    for name, body in definitions.items():
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(body)
    result = {'passed': False, 'case': args.case, 'definitions': definitions, 'references': references, 'fixture': str(base), 'native_version': '1.0.83',
    ...
    def native(label, workspace):
        ...
        assert all(value == (name != '.claude/CLAUDE.md' or args.case != 'combined') for name, value in phase['instruction_markers'].items()), phase['instruction_markers']
        expected_references = {'root-policy.md', 'child-policy.md', '.github/policy.md', '.claude/policy.md' if args.case == 'fallback' else 'claude-policy.md'}
        if args.case == 'dot-only': expected_references = {'.claude/policy.md'}
