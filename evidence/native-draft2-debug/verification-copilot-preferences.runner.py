...
def main():
    ...
    local_label = 'selection-preferences-reviewed' if args.with_preferences else 'selection-legacy-reviewed' if args.with_legacy else 'selection-mcp-reviewed' if args.with_mcp else 'selection-git-reviewed' if args.with_git else 'selection-verified'
    ...
    if args.with_git:
        git_label = 'git-preferences-reviewed' if args.with_preferences else 'git-legacy-reviewed' if args.with_legacy else 'git-mcp-reviewed' if args.with_mcp else 'git-verified'
    ...
    if args.with_legacy:
        label = 'preferences' if args.with_preferences else 'final'
    ...
    if args.with_preferences:
        native.append('WORKBENCH/evidence/native-draft2-debug/copilot-preferences-final.json')
    ...
    def native_records():
        ...
        for name in native:
            ...
            runner = 'run_native_copilot_preferences.py' if Path(name).name == 'copilot-preferences-final.json' else 'run_native_copilot_legacy.py' if Path(name).name.startswith('copilot-legacy-') else 'run_native_plugin_mcp.py' if '-mcp-final-' in name else 'run_native_plugin_git.py' if Path(name).name.startswith(('codex-git-', 'copilot-git-')) else 'run_native_plugin_selections.py'
