#!/usr/bin/env python3
"""Check canonical instruction references and distinct native bodies through relocation."""
import argparse
...
def main():
    ...
    parser.add_argument('--case', choices=['link-only', 'link-distinct'], required=True)
    ...
    (source/'.agents').mkdir()
    (source/'.agents/manifest.json').write_text(json.dumps({'version': '1.1.0-draft.2', 'profiles': []}))
    (source/'.agents/AGENTS.md').write_text(body)
    (source/'.agents/policy.md').write_text('AGENTS_CANONICAL_REFERENCE_DECOY\n')
    (source/'AGENTS.md').symlink_to('.agents/AGENTS.md')
    native_body = 'AGENTS_OTHER_INSTRUCTION_MARKER\n@native-policy.md\n'
    if args.case == 'link-distinct':
    ...
    def native(label, workspace):
        ...
        assert 'AGENTS_REFERENCED_POLICY' in context and 'AGENTS_CANONICAL_REFERENCE_DECOY' not in context
        assert 'AGENTS_WRONG_REFERENCE_BASE' not in context
        assert ('AGENTS_OTHER_INSTRUCTION_MARKER' in context) == (args.case == 'link-distinct')
        assert ('AGENTS_NATIVE_REFERENCED_POLICY' in context) == (args.case == 'link-distinct')
        assert ('AGENTS_UPDATED_CORE_MARKER' in context) == (label == 'updated')
    ...
    def adapter(operation, workspace, expect=0):
        ...
        if operation == 'apply' and workspace == source: argv += ['--adopt']
        return command(argv, workspace, expect)

    try:
        native('source', source)
        command(['go', 'build', '-trimpath', '-buildvcs=false', '-o', base/'agents', './cmd/agents'], repo/'CLI')
        before = {str(p.relative_to(source)): sha(p) for p in source.rglob('*') if p.is_file() and p != source/'.agents/manifest.json'}
        authority = sha(home/'config.json')
        adapter('import', source)
        assert (source/'.agents/AGENTS.md').read_text() == body
        if args.case == 'link-distinct':
            assert (source/'.agents/native/com.github.copilot/copilot-instructions.md').read_text() == native_body
        shutil.copytree(source/'.agents', target/'.agents')
        link_before = os.lstat(source/'AGENTS.md')
        adapter('apply', source)
        link_after = os.lstat(source/'AGENTS.md')
        result['source_link_preserved'] = (link_before.st_dev, link_before.st_ino) == (link_after.st_dev, link_after.st_ino)
        assert result['source_link_preserved']
        result['plan'] = json.loads(adapter('plan', target)['stdout'])
        adapter('apply', target)
        assert (target/'AGENTS.md').read_text() == body
        assert not (target/'AGENTS.md').is_symlink()
        if args.case == 'link-distinct': assert (target/'.github/copilot-instructions.md').read_text() == native_body
        else: assert not (target/'.github/copilot-instructions.md').exists()
        native('relocated', target)
        adapter('import', target)
        assert (target/'.agents/AGENTS.md').read_text() == body
        if args.case == 'link-distinct':
            assert (target/'.agents/native/com.github.copilot/copilot-instructions.md').read_text() == native_body
        assert (source/'AGENTS.md').is_symlink() and os.readlink(source/'AGENTS.md') == '.agents/AGENTS.md'
        updated = body+'AGENTS_UPDATED_CORE_MARKER\n'
        (target/'.agents/AGENTS.md').write_text(updated)
        adapter('apply', target)
        assert (target/'AGENTS.md').read_text() == updated
        native('updated', target)
        adapter('import', target)
        assert (target/'.agents/AGENTS.md').read_text() == updated
        if args.case == 'link-distinct': assert (target/'.github/copilot-instructions.md').read_text() == native_body
