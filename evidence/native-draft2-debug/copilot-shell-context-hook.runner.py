...
def main():
 ...
 parser.add_argument('--paths',choices=['all','workspace'],default='all')
 ...
 common=[str(binary),'--disable-builtin-mcps','--no-auto-update','--no-remote','--no-remote-export','--no-bash-env','--no-custom-instructions',*(['--allow-all-paths'] if args.paths=='all' else []),'--log-level','debug',*flags]
