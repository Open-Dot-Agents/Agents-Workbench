...
def main():
 ...
 (directory/'profile.json').write_text(json.dumps({'namespace':'com.github.copilot','harnessVersion':'=1.0.83','scope':args.scope,'required':True,'artifacts':[{'kind':'lsp','source':'lsp.json'}]}))
 config={'lspServers':{'fixture':{'command':'${ODA_LSP_PYTHON}','args':[str(server),'${ODA_LSP_EVENTS}'],'fileExtensions':{'.oda':'fixture'},'env':{'ODA_LSP_MODE':'${ODA_LSP_INPUT}'},'initializationOptions':{'sentinel':'ODA_LSP','exact':9007199254740993},'rootUri':'packages/frontend','requestTimeoutMs':30000}}}
 ...
 command=[str(cli),'apply','copilot',str(canonical),'--experimental','--scope',args.scope]
