...
def main():
    ...
    parser.add_argument('--case', choices=['provider', 'shell', 'skills'], default='provider')
...
'''
    ...
    control_key = {'provider': 'model_provider', 'shell': 'features.shell_tool', 'skills': 'skills.include_instructions'}[args.case]
    if args.case == 'shell':
        user_config = user_config.replace('[features]\n', '[features]\nshell_tool=false\n')
    elif args.case == 'skills':
        user_config = control_key + '=false\n' + user_config
    if args.case == 'skills':
        skill = workspace / '.agents/skills/fixture/SKILL.md'
...
'''
    ignored_role = role + ("model_provider='child'\n" if args.case == 'provider' else control_key+'=true\n')
    if args.case != 'provider': role += control_key+'=false\n'
    ...
    try:
        ...
        flags = ['--vendor', 'codex', '--root', workspace, '--experimental', '--scope', args.scope]
        if args.scope == 'user': flags += ['--native-home', home]
        command([cli, 'import', *flags])
        canonical = workspace / '.agents/native/com.openai.codex/agent/fixture.toml'
        assert canonical.read_text() == ignored_role
        ...
        def snapshot():
            return {str(p.relative_to(base)): [p.stat().st_mode & 0o777, sha(p) if p.is_file() else 'directory']
                    for folder in (workspace, home, base / 'state') if folder.exists() for p in folder.rglob('*')}
        before = snapshot()
        ...
        assert not target.exists() and canonical.read_text() == ignored_role
        result['optional_preserved_inactive'] = True
        canonical.write_text(role)
        profile['required'] = True; profile_path.write_text(json.dumps(profile))
        if args.case != 'provider':
            setting = 'shell_tool' if args.case == 'shell' else control_key
            user_config = user_config.replace(setting+'=false\n', setting+'=true\n')
