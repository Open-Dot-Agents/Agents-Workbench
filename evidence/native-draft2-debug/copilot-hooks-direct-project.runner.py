...
def main():
 ...
 effect=root/'effect.txt';probe=root/'effect.py';probe.write_text('from pathlib import Path\nPath('+repr(str(effect))+').write_text("ODA_HOOK_TOOL_EFFECT")\n')
 ...
 try:
  ...
  if args.scenario=='disabled':assert not events,'disabled file hooks ran'
   ...
   assert pre['env']=={'ODA_LITERAL':'fixture','ODA_EXPANDED':'expanded-fixture'},'hook environment mismatch'
  ...
  if args.scenario=='deny':
   ...
   assert not client.approvals,'denial reached approval prompt'
  else:
   assert effect.read_text()=='ODA_HOOK_TOOL_EFFECT','tool effect absent'
   assert any(e['approved'] and e['params'].get('toolCall',{}).get('toolCallId')=='fixture-hook-tool' for e in client.approvals),'no correlated native approval'
