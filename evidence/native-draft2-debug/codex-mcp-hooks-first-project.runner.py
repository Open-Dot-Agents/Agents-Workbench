...
def main():
    ...
    parser.add_argument('--scenario', choices=['execution', 'deny', 'missing-server',
                        'missing-tool', 'error', 'timeout', 'unreviewed', 'template-missing'],
                        default='execution')
    ...
    def group(event, matcher='Bash'):
        inputs = {'event': '${hook_event_name}', 'session': '${session_id}',
                  ...
                  'nested': [{'name': '${tool_name}'}, 7, True, None],
    ...
    try:
        ...
        if args.scenario == 'unreviewed':
            ...
            for call in calls:
                ...
                assert values['nested'] == [{'name': 'Bash'}, 7, True, None], 'recursive template mismatch'
