...
def main():
    ...
    def adapter(operation, workspace, label, extra=()):
        return command([base/'agents', operation, '--vendor', 'copilot', '--root', workspace, '--experimental',
    ...
    try:
        ...
        authority = {label: sha(home/'config.json') for label, home in native_homes.items()}
        ...
        result['authority_unchanged'] = authority == {label: sha(home/'config.json') for label, home in native_homes.items()}
