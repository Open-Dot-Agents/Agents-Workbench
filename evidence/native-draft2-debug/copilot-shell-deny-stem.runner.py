...
Native path access is pregranted to keep this test specific to shell rules.
...
def main():
 ...
 parser.add_argument('--command',choices=['python-absolute','python-name','touch-absolute','touch-name','compound'],default='python-absolute')
 parser.add_argument('--rule',choices=['absolute-exact','basename-exact','absolute-prefix','basename-prefix','absolute-stem','basename-stem','all','none','all-deny-exact','all-deny-stem'],required=True)
 ...
 common=[str(binary),'--disable-builtin-mcps','--no-auto-update','--no-remote','--no-remote-export','--no-bash-env','--no-custom-instructions','--allow-all-paths','--log-level','debug',*flags]
 ...
 try:
  if args.interface=='prompt':
   ...
   assert len(terminal)==1,'no single correlated terminal event'
   executed=terminal[0]['status']=='completed';denied=terminal[0]['status']=='failed'
  ...
  assert len(requests)>=2,'tool result did not return to model'
