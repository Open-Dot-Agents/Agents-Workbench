...
def fixture_edit_allowed(params, path, session_id):
    ...
    return (params.get('sessionId') == session_id and call.get('toolCallId') == 'trigger-file'
            and call.get('kind') == 'edit' and call.get('locations') == [{'path': str(path)}]
            and call.get('rawInput') == {'fileName': str(path), 'diff': expected})
...
def main():
    ...
    class FixtureClient(Client):
        def send(self, message):
            if 'result' in message and self.events and self.approvals:
                ...
                if (args.trigger == 'edit' and event.get('method') == 'session/request_permission'
                        and event.get('id') == message.get('id')
                        and fixture_edit_allowed(params, active, record.get('session', {}).get('sessionId'))):
    ...
    client = FixtureClient([str(binary), '--acp', '--disable-builtin-mcps', '--no-auto-update', '--no-remote'], workspace, env, 'deny', '')
    try:
