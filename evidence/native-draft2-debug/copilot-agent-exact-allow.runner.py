...
(b/'effect.py').write_text("from pathlib import Path\nPath("+repr(str(b/'effect.txt'))+").write_text('ODA_NATIVE_CHILD_EFFECT')\n")
requests=[]
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  if n==1:
   ...
   if bash:call=('bash',{'command':f"/usr/bin/python3 {b}/effect.py",'description':'Write isolated fixture marker'})
  if call:
...
c=Client([binary,'--acp','--disable-builtin-mcps','--no-auto-update','--no-remote'],w,env,'allow',str(b/'effect.py'));r={'fixture':str(b),'native_version':'1.0.83','sha256':sha(binary)};d=time.monotonic()+35
try:
...
r.update(events=c.events,approvals=c.approvals,stderr=c.errors,requests=requests,marker=(b/'effect.txt').read_text() if (b/'effect.txt').exists() else '')
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-agent-exact-allow.json');assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print('RESULT',r.get('error'),r.get('prompt'),r['marker'])
