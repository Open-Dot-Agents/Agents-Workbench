...
def main():
    ...
    try:
        ...
        if args.case in ('distinct', 'reference'):
            refusal = adapter('import', source, 1)
            assert ('distinct project instructions' if args.case == 'distinct' else 'reference') in refusal['stderr']
            assert not (source/'.agents').exists()
            if args.case == 'reference':
                # A manual unconverted copy tests the native reference base.
                # The adapter must refuse this conversion.
                (target/'.github/copilot-instructions.md').write_text(body)
                native('unconverted', target)
        else:
            ...
            assert (target/'.github/copilot-instructions.md').read_text() == body
