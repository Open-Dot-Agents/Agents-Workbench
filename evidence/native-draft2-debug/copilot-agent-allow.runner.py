...
c=Client([binary,'--acp','--disable-builtin-mcps','--no-auto-update','--no-remote'],w,env,'allow','');r={'fixture':str(b),'native_version':'1.0.83','sha256':sha(binary)};d=time.monotonic()+35
try:
...
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-agent-allow.json');assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print('RESULT',r.get('error'),r.get('prompt'),r['marker'])
