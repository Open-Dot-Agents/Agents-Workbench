...
while True:
 ...
 if 'id' in message:
  ...
  if message.get('method')=='textDocument/hover' and len(sys.argv)>2 and sys.argv[2]=='timeout':time.sleep(5)
...
def main():
 ...
 parser.add_argument('--scenario',choices=['hover','timeout'],default='hover')
 ...
 try:
  ...
  if args.scenario=='hover':
   ...
   assert completed and any('timed out' in json.dumps(e).lower() or 'timeout' in json.dumps(e).lower() for e in completed),'no correlated native timeout event'
