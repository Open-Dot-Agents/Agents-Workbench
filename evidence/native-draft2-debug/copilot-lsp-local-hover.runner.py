...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  if len(requests)==1:
   assert any(t['function']['name']=='lsp' for t in tools)
   msg={'role':'assistant','content':None,'tool_calls':[{'id':'fixture-lsp-call','type':'function','function':{'name':'lsp','arguments':json.dumps({'operation':'hover','file':str(w/'test.oda'),'line':1,'character':1})}}]};finish='tool_calls'
...
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-lsp-local-hover.json');assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print('RESULT',r.get('error'),r.get('prompt'),r['lsp_events'])
