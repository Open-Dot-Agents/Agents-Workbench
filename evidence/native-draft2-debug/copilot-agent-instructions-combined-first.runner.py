...
def main():
    ...
    parser.add_argument('--case', choices=['combined', 'fallback'], required=True)
    ...
    def native(label, workspace):
        ...
        assert all(value == (name != '.claude/CLAUDE.md' or args.case == 'fallback') for name, value in phase['instruction_markers'].items()), phase['instruction_markers']
    ...
    try:
        ...
        assert (source/'.agents/AGENTS.md').read_text() == definitions['.github/copilot-instructions.md']
        for name, body in definitions.items():
        ...
        assert (target/'.agents/AGENTS.md').read_text() == definitions['.github/copilot-instructions.md']
        for name, body in definitions.items():
            if name != '.github/copilot-instructions.md':
