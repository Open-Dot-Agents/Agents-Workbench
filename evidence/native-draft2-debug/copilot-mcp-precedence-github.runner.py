...
if opt.scope=='github':
 high=json.loads(json.dumps(config));high['mcpServers']['oda-fixture']['args'][1]=str(b/'higher-events.jsonl')
 (w/'.mcp.json').write_text(json.dumps(high))
...
r.update(events=c.events,approvals=c.approvals,stderr=c.errors,requests=requests,higher_events=(b/'higher-events.jsonl').read_text() if (b/'higher-events.jsonl').exists() else '',mcp_events=(b/'mcp-events.jsonl').read_text() if (b/'mcp-events.jsonl').exists() else '')
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-mcp-precedence-'+opt.scope+('-experimental' if opt.experimental else '')+'.json');assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print('RESULT',r.get('error'),r.get('prompt'),r['mcp_events'])
