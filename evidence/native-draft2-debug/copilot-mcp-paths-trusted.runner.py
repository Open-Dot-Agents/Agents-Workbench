...
for name,relative in [('github','.github/mcp.json'),('root','.mcp.json'),('user','mcp-config.json')]:
 ...
 path=(home if name=='user' else work)/relative;path.parent.mkdir(parents=True,exist_ok=True);(home/'settings.json').write_text(json.dumps({'trustedFolders':[str(work)],'experimental':True}))
 path.write_text(json.dumps({'mcpServers':{'oda-'+name:{'type':'local','command':'/usr/bin/false','args':[],'tools':['*']}}}))
...
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-mcp-paths-trusted.json');assert not p.exists();p.write_text(json.dumps({'native_version':'1.0.83','native_sha256':sha,'fixture':str(b),'cases':cases},indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print(json.dumps(cases,indent=2))
