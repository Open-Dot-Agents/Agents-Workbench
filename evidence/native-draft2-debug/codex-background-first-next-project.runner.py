...
child = None
if scenario in ['background-timeout', 'background-cancel']:
...
def main():
    ...
    parser.add_argument('--scenario', choices=['background-next', 'background-active',
                        'background-deny', 'background-timeout', 'background-cancel',
                        'spill', 'unlimited', 'spill-default', 'spill-mixed'], default='background-next')
    ...
    handler = {'type': 'command', 'command': hook_command, 'timeout': 15,
               'async': background, 'additionalContextLimit': 0 if background or args.scenario == 'unlimited' else 32}
    ...
    def check_execution():
        ...
        if background:
            ...
            if args.scenario in ['background-next', 'background-deny']:
                ...
                gate.touch()
            if args.scenario == 'background-cancel':
                ...
                while process_state(initial[0]['pid']) not in [None, 'Z'] and time.monotonic() < stop_deadline:
                    time.sleep(0.02)
                result['process_states_after_unsubscribe'] = {
                    str(pid): process_state(pid) for pid in [initial[0]['pid'], initial[0]['child_pid']]}
                assert all(state in [None, 'Z'] for state in result['process_states_after_unsubscribe'].values()), 'native unsubscribe left hook processes alive'
                assert len(hook_log()) == 1, 'cancelled hook completed'
                assert not log.with_suffix('.child-effect').exists(), 'cancelled child effect occurred'
            else:
                expected_runs = 2 if args.scenario == 'background-active' else 1
                completion_deadline = time.monotonic() + 10
                while len([e for e in client.events if e.get('method') == 'hook/completed']) < expected_runs:
                    client.receive(completion_deadline)
                if args.scenario == 'background-timeout':
            ...
            if args.scenario == 'unlimited':
                ...
                if args.scenario == 'spill-mixed':
                    ...
                    assert context not in received and 'ODA_CONTEXT_TAIL' not in received, 'context was not shortened'
        ...
        assert runs or args.scenario == 'background-cancel', 'native hook completion absent'
        for entry in runs:
            run = entry['run']
            ...
            elif args.scenario != 'background-cancel':
