...
def main():
    ...
    for name, body in definitions.items():
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(body)
    result = {'passed': False, 'definitions': definitions, 'references': references, 'fixture': str(base), 'native_version': '1.0.83',
    ...
    def native(label, workspace):
        ...
        assert all(phase['instruction_markers'].values()), phase['instruction_markers']
        assert all(value == (name != 'gemini-policy.md') for name, value in phase['reference_markers'].items()), phase['reference_markers']
