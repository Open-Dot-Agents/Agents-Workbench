...
def main():
 ...
 try:
  ...
  if args.interface=='prompt':
   command=[str(binary),'--disable-builtin-mcps','--no-auto-update','--no-remote','--allow-tool','shell(/usr/bin/python3)','--allow-all-paths','--stream','off','--output-format','json','-p','Run the isolated hook fixture command.'];result['native_command']=command
