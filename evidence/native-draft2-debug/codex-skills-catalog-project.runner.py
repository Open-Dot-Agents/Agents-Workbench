...
def main():
    ...
    def discover(native_home, path, enabled, phase):
        ...
        try:
            ...
            assert record['catalog_contains_skill'] is enabled, 'model catalog does not follow skill enablement'
            if args.scope == 'user': assert record['discovery_matches_setting'], 'user skill discovery did not follow enablement'
        finally:
            client.close()
