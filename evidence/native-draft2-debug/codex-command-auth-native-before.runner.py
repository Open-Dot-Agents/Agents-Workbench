...
def main():
    ...
    parser.add_argument('--case', choices=['success', 'timeout', 'empty', 'exit', 'invalid-utf8', 'missing'], default='success')
...
print('oda-command-synthetic-token')
''')
    helper.chmod(0o600)
    ...
    class Model(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            requests.append({'path': self.path, 'body': body, 'authorized': self.headers.get('Authorization') == 'Bearer oda-command-synthetic-token'})
...
'''+f'base_url = "http://127.0.0.1:{server.server_port}/v1"\n[model_providers.fixture.auth]\ncommand = '+json.dumps('/missing/oda-token-command' if args.case == 'missing' else '/usr/bin/python3')+'\nargs = '+json.dumps(['token.py', args.case, 'literal;argument', '$(literal)'])+'\ncwd = '+json.dumps(str(command_dir))+'\ntimeout_ms = 100\nrefresh_interval_ms = 0\n'
        ...
        assert len(calls()) == count and sha(helper) == helper_before
        assert not list(owner.rglob('token.py')) and not list(again.rglob('token.py'))
        assert not list(root.rglob('auth.json')), 'native account file was created'
