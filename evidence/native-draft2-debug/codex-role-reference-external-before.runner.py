...
def main():
    ...
    parser.add_argument('--reference', choices=['external', 'managed', 'absolute'], required=True)
    ...
    result = {'passed': False, 'reference': args.reference, 'fixture': str(base), 'native_version': '0.154.0',
    ...
    role = source / ('agents' if args.reference == 'managed' else 'role-library') / 'fixture.toml'
...
''')
    ...
    reference = str(role) if args.reference == 'absolute' else str(role.relative_to(source))
...
'''
    ...
    source_config_hash = sha(source / 'config.toml')
    ...
    def native(label, home):
        ...
        start = len(requests)
        client = Client([str(binary), 'app-server', '--listen', 'stdio://'], workspace, {**env, 'CODEX_HOME': str(home)}, 'deny', '')
    ...
    try:
        ...
        flags = ['--vendor', 'codex', '--experimental', '--scope', 'user']
        command([cli, 'import', *flags, '--root', canonical, '--native-home', source])
        ...
        command([cli, 'apply', *flags, '--root', canonical, '--native-home', target])
        result['projected_config'] = tomllib.loads((target / 'config.toml').read_text())
        ...
        again = base / 'again'
        command([cli, 'import', *flags, '--root', again, '--native-home', target])
        ...
        assert sha(role) == original_role_hash and sha(source / 'config.toml') == source_config_hash
        assert not (target / 'role-library').exists()
        if args.reference == 'managed': assert sha(target / 'agents/fixture.toml') == original_role_hash
