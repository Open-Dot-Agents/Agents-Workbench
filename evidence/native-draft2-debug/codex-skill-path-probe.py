...
for kind in ('absolute-file','absolute-folder','relative-file','relative-folder'):
 ...
 path=str(skill) if kind=='absolute-file' else str(skill.parent) if kind=='absolute-folder' else 'skills/fixture/SKILL.md' if kind=='relative-file' else 'skills/fixture'
 (home/'config.toml').write_text('[[skills.config]]\npath = '+json.dumps(path)+'\nenabled = false\n')
 c=Client([binary,'app-server','--listen','stdio://'],workspace,{'PATH':'/usr/bin:/bin','HOME':str(home/'isolated-home'),'CODEX_HOME':str(home)},'deny',str(root/'unused'))
...
print(json.dumps({'fixture':str(root),'cases':[{k:v for k,v in r.items() if k in ('case','error','fixture_skills')} for r in result['cases']]},indent=2));p=pathlib.Path('/mnt/DATA/tmp/oda-codex-skill-path-probe.json');assert not p.exists();p.write_text(json.dumps(result,indent=2)+'\n')
