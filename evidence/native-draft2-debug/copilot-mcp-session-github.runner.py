...
(h/'settings.json').write_text(json.dumps({'trustedFolders':[str(w)]}))
requests=[]
...
p=pathlib.Path('WORKBENCH/evidence/native-draft2-debug/copilot-mcp-session-'+opt.scope+('-experimental' if opt.experimental else '')+'.json');assert not p.exists();p.write_text(json.dumps(r,indent=2)+'\n');p.with_suffix('.runner.py').write_bytes(pathlib.Path(__file__).read_bytes());print('RESULT',r.get('error'),r.get('prompt'),r['mcp_events'])
