    ...
    try:
        ...
        if args.vendor == 'codex':
            records = [s for item in enabled['data'] if item['cwd'] == str(workspace)
                       for s in item['skills'] if s['name'] == 'fixture']
            assert len(records) == 1 and records[0]['scope'] == 'plugin' and records[0]['enabled'] is True
            assert all(not item['errors'] for item in enabled['data'])
            installed_skill = Path(records[0]['path'])
            installed_package = home / 'plugins/cache/oda-fixture/fixture/local'
        else:
            records = [p for p in enabled['pluginRecords'] if p['spec'] == 'fixture@oda-fixture']
            assert len(records) == 1 and records[0]['name'] == 'fixture'
            assert records[0]['marketplace'] == 'oda-fixture' and records[0]['version'] == '1.0.0'
            assert records[0]['enabled'] is True and records[0]['installed'] is True
            assert records[0]['disabledReason'] is None
            installed_package = home / 'installed-plugins/oda-fixture/fixture'
            assert Path(records[0]['installPath']) == installed_package
            skills = [s for s in enabled['skillRecords'] if s['pluginId'] == 'fixture@oda-fixture']
            assert len(skills) == 1 and skills[0]['name'] == 'fixture'
            assert skills[0]['source'] == 'plugin' and skills[0]['enabled'] is True
            assert skills[0]['disabledReason'] is None
            installed_skill = Path(skills[0]['path'])
        ...
        if args.vendor == 'codex':
            assert not any(s['name'] == 'fixture' for item in disabled['data'] for s in item['skills'])
            assert all(not item['errors'] for item in disabled['data'])
        else:
            records = [p for p in disabled['pluginRecords'] if p['spec'] == 'fixture@oda-fixture']
            assert len(records) == 1 and records[0]['enabled'] is False and records[0]['installed'] is True
            assert records[0]['disabledReason'] == 'disabled-by-spec'
            assert disabled['skillRecords'] == []
