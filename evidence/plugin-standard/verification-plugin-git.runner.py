...
def main():
    ...
    local_label = 'selection-git-reviewed' if args.with_git else 'selection-verified'
    ...
    if args.with_git:
        native += [f'WORKBENCH/evidence/plugin-standard/{vendor}-git-verified-{scope}.json'
                   for vendor in ('codex', 'copilot') for scope in ('project', 'user')]
    ...
    def native_records():
        ...
        for name in native:
            ...
            runner = 'run_native_plugin_git.py' if '-git-verified-' in name else 'run_native_plugin_selections.py'
        ...
        return {'records': native, 'shared_package_hashes': packages[0]}
