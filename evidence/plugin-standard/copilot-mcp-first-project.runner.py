...
def main():
    ...
    root = Path(tempfile.mkdtemp(prefix='oda-plugin-mcp-', dir='/mnt/DATA/tmp'))
    ...
    (plugin / 'mcp.json').write_text(json.dumps({'$schema': 'https://agent-plugins.org/schemas/1.0.0/mcp.schema.json',
        'mcpServers': {'fixture-mcp': {'type': 'stdio', 'command': 'python3',
            'args': ['${PLUGIN_ROOT}/server.py', '${PLUGIN_DATA}/events.jsonl'], 'cwd': '${PLUGIN_DATA}',
    ...
    result = {'vendor': args.vendor, 'scope': args.scope, 'fixture': str(root), 'native_sha256': sha(binary),
    ...
    class Model(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            names = [t.get('function', t).get('name', '') for t in request.get('tools', [])]
            matches = [name for name in names if name.endswith('portable_record')]
            ...
            if call:
                ...
                message = {'role': 'assistant', 'content': None, 'tool_calls': [{'id': 'fixture-portable-call',
                    'type': 'function', 'function': {'name': matches[0], 'arguments': arguments}}]}
                item = {'type': 'function_call', 'call_id': 'fixture-portable-call', 'name': matches[0], 'arguments': arguments}
    ...
    def session():
        command = [str(binary), 'app-server', '--stdio'] if args.vendor == 'codex' else [str(binary), '--acp',
            '--disable-builtin-mcps', '--no-auto-update', '--no-remote', '--allow-tool', 'fixture-mcp(portable_record)']
    ...
    try:
        if args.vendor == 'codex':
            config = {'model': 'fixture-model', 'model_provider': 'fixture', 'approval_policy': 'never', 'sandbox_mode': 'workspace-write',
        ...
        session()
        logs = list(home.rglob('events.jsonl'))
        ...
        assert process['env']['ODA_UNKNOWN'] == '${ODA_AMBIENT}', 'native expanded a nonstandard placeholder'
        assert (data / 'effect.txt').read_text() == 'ODA_PORTABLE_EFFECT', 'MCP tool produced no effect'
        call = next(e for e in events if e.get('method') == 'tools/call')
        assert call['params'] == {'name': 'portable_record', 'arguments': {'marker': 'ODA_PORTABLE_CALL'}}, 'MCP call differs'
        ...
        assert any(e.get('id' if args.vendor == 'codex' else 'toolCallId') == 'fixture-portable-call' and e.get('status') == 'completed' and 'ODA_PORTABLE_RESULT' in json.dumps(e) for e in completed), 'native did not report correlated MCP completion'
        ...
        assert disabled and not any('portable_record' in t.get('function', t).get('name', '') for r in disabled for t in r.get('tools', [])), 'disabled plugin tool is still exposed'
        assert sha(log) == log_hash, 'disabled plugin started its MCP server'
        assert (data / 'effect.txt').read_text() == 'ODA_PORTABLE_EFFECT', 'disable removed plugin data'
        ...
        result['mcp_logs'] = {str(p): p.read_text() for p in home.rglob('events.jsonl')}
        http.shutdown()
        http.server_close()
