...
class PluginClient(Client):
    def send(self, message):
        ...
        if ('error' in message and event.get('id') == message.get('id') and
            ...
            self.approvals.append({'method': event['method'], 'params': params, 'response': response, 'approved': approved})
...
def main():
    ...
    result = {'vendor': args.vendor, 'scope': args.scope, 'native_approval': args.native_approval, 'fixture': str(root), 'native_sha256': sha(binary),
    ...
    def session():
        ...
        client = PluginClient(command, workspace, env, 'plugin-allow', '')
        try:
